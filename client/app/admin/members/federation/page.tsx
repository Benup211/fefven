...
function FederationMemberContent() {
    const searchParams = useSearchParams();
    const { members, fetchMembers } = useMemberStore();
    const status =
        (searchParams.get("status") as EnrollmentStatus) ||
        ...
        return members.filter(
            (member) =>
                (member.name.toLowerCase().includes(search.toLowerCase()) ||
                    member.contactNo.includes(search)) &&
                (status === EnrollmentStatus.ALL ||
                    member.enrollmentStatus === status)
        );
    };
...
export default function FederationMemberList() {
    return (
        <Suspense fallback={<FederationMembersSkeleton />}>
            <FederationMemberContent />
        </Suspense>
    );
}
