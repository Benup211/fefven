...
interface DistrictMembersState {
...
export const useDistrictMembersStore = create<DistrictMembersState>((set) => ({
    members: [],
    isLoading: false,
    error: null,
    fetchMembers: async () => {
        ...
        try {
            const response = await axios.get(
                `${backendUrl}/api/district-member/all`
            );
    ...
    addMember: async (member) => {
        ...
        try {
            const response = await axios.post(
              `${backendUrl}/api/district-member/create`,
              member,
              {
                  headers: {
    ...
    deleteMember: async (id) => {
      ...
      try {
          await axios.delete(
              `${backendUrl}/api/district-member/delete/${id}`
          );
          set((state) => ({
