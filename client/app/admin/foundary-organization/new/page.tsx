...
import { useFoundingOrganizationStore } from "@/state/admin/founding-organization-store";
import { ArrowLeft, Loader2 } from "lucide-react";
import Link from "next/link";
import { toast } from "@/hooks/use-toast";

export default function CreateFoundingOrganization() {
    const [name, setName] = useState("");
    const [establishedAt, setEstablishedAt] = useState("");
    const router = useRouter();
    const { createOrganization, Loading } = useFoundingOrganizationStore();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (response.success) {
            ...
            router.push("/admin/foundary-organization");
        } else {
            toast({
        ...
        <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 dark:bg-gray-900">
            <div className="w-full max-w-md bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
                <div className="flex justify-between items-center mb-6">
                    <Link href="/admin/foundary-organization" passHref>
                        <Button
                            variant="ghost"
                            size="icon"
